...
const DayView = ({ token, id }) => {
  ...
  const updateSelected = (id, day) => {
    if (recordSelected.indexOf(id) > -1) {
      setRecordSelected((prevState) =>
        prevState.filter((sessionId) => sessionId != id)
      );
      setMoveDay((prevState) =>
        prevState.filter((sessionId) => sessionId.id != id)
      );
    } else {
  ...
  const [moveSessionData, { isSuccess: moveSuccess, isError: moveError }] =
    useMoveSessionMutation();
  ...
  const onSubmit = (data) => {
    if (data?.singleViewAction === "") {
      ...
      if (data?.singleViewAction == 1) {
        let dayArray = [];
        moveDay.map((dayChecked) =>
          dayArray.indexOf(dayChecked.day) === -1
            ? dayArray.push(dayChecked.day)
            : ""
        );
